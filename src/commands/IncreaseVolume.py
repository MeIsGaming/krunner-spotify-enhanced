...
class IncreaseVolume(Command):
    ...
    def Match(self, query: str):
        if IncreaseVolume.plusCount == 0:
            IncreaseVolume.currentVolume = self.spotify.current_playback()["device"][
                "volume_percent"
            ]
        ...
        if query.isnumeric() and len(query) > 0:
            ...
            len(query) > 0
            and query[0] == getSetting("INCREASE_VOLUME_CHARACTER")
    ...
    def Run(self, data: str):
        ...
        IncreaseVolume.currentVolume = (
            IncreaseVolume.currentVolume if IncreaseVolume.currentVolume < 100 else 100
        )
        self.spotify.volume(IncreaseVolume.currentVolume)
    ...
    def IncreaseByPlusCharacter(self, query):
        ...
        if 0 == abs(plusDifference) or abs(plusDifference) == 1:
            if plusDifference == 0:
                plusDifference = 1
            IncreaseVolume.currentVolume = (
                IncreaseVolume.currentVolume + int(getSetting("VOLUME_STEP")) * plusDifference
            )
            ...
            if IncreaseVolume.currentVolume < 0:
                ...
                return [
                    (
                        "",
                        "Volume is now set to: " + str(IncreaseVolume.currentVolume),
                        "Spotify",
                        100,
                        100,
                        {},
