...
class SetVolume(Command):
    ...
    def Match(self, query: str):
        ...
        if query.isnumeric() and len(query) > 0:
            ...
            decreaseCharacters = query.count(getSetting("DECREASE_VOLUME_CHARACTER"))
            increaseCharacters = query.count(getSetting("INCREASE_VOLUME_CHARACTER"))
            ...
            if decreaseCharacters > SetVolume.previousDecreaseCharacters:
                ...
                ) * abs(decreaseCharacters - SetVolume.previousDecreaseCharacters)
            elif increaseCharacters > SetVolume.previousIncreaseCharacters:
                SetVolume.currentVolume = SetVolume.currentVolume + int(
                    getSetting("VOLUME_STEP")
                ) * abs(increaseCharacters - SetVolume.previousIncreaseCharacters)
            ...
            if SetVolume.currentVolume < 0:
                ...
                returnOptions = [
                    (
                        " ",
                        "Volume has been set to: " + str(SetVolume.currentVolume) + "%",
                        "Spotify",
                        100,
                        100,
                        {},
    ...
    def setVolumeByValue(self, query):
        ...
        if volume < 0 or volume > 100:
            ...
            return [
                (
                    self.command + " " + str(volume),
                    "Increase volume with " + str(volume) + "%",
                    "Spotify",
                    100,
                    100,
                    {},
    ...
    def setVolumeByChoice(self):
        return [
            (self.command + " 10", "Set volume to 10%", "Spotify", 100, 100, {}),
            (self.command + " 25", "Set volume to 25%", "Spotify", 100, 80, {}),
            (self.command + " 50", "Set volume to 50%", "Spotify", 100, 60, {}),
