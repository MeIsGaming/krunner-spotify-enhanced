...
def _ensure_test_config() -> None:
    ...
    (config_dir / "KRunner-Spotify.config").write_text(TEST_CONFIG, encoding="utf-8")
...
def test_command_names_are_unique_and_non_empty():
    commands = _load_commands_module()
    names = commands.getCommandNames()
    assert names
    assert len(names) == len(set(names))


def test_autocomplete_filters_by_prefix():
    commands = _load_commands_module()
    matches = commands.autocompleteMatches("PA")
    assert matches
    assert all(entry[0].startswith("PA") for entry in matches)


def test_execute_unknown_command_raises_runtime_error():
    commands = _load_commands_module()
    spotify = MagicMock()
    try:
        commands.executeCommand("UNKNOWN", spotify)
        assert False, "Expected RuntimeError for unknown command"
    except RuntimeError:
        assert True
